...
export class GPIOService {
  private buttons: { [key: string]: any } = {};
    ...
    try {
      ...
      for (const [buttonName, pin] of Object.entries(buttonPins)) {
        try {
          ...
          this.buttons[buttonName].watch((err: any, value: any) => {
            if (err) {
          ...
          console.error(
            `GPIO Service: Failed to initialize ${buttonName} on pin ${pin}:`,
            error
          );
        }
      }
  ...
  private setupPythonBridge() {
        ...
        if (fs.existsSync(commFile)) {
          ...
          if (currentFileModTime > lastFileModTime) {
            ...
            if (
              ...
              console.log(
                `GPIO Service: Processing button event: ${eventData.button}`
              );
    ...
    console.log(
      'GPIO Service: Make sure to run: sudo python3 /home/pi/gpio_buttons.py'
    );
  }

  private setupKeyboardSimulation() {
  ...
  private sendButtonEvent(buttonName: string) {
    if (!this.mainWindow) return;
