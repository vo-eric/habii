...
export const WebSocketProvider: React.FC<WebSocketProviderProps> = ({
  ...
  useEffect(() => {
    ...
    const initializeSocket = async () => {
      ...
      try {
        // Check if this is a pre-authenticated user (desktop mode)
        if (
          user.uid.startsWith('desktop-user-') ||
          user.uid === '6wbqAvVg9VaVBC5Ywgyolv2hi5M2'
        ) {
          console.log(
            'Pre-authenticated user detected, using development token'
          );
          token = 'dev-token';
        } else {
          // Real Firebase user - get ID token
